...
def test_organise_items(gilded_rose):
    '''Test that items are organised into relevant categories'''
    assert gilded_rose.organise_items()["General Items"] is not None
...
#     assert gilded_rose.items[8].sell_in == 2
...
#     items = [Item(name="+5 Dexterity Vest", sell_in=10, quality=0), Item(name="Elixir of the Mongoose", sell_in=5, quality=0), Item(name="Conjured Mana Cake", sell_in=3, quality=0)]
#     gilded = GildedRose(items)
#     gilded.update_quality()
#     assert gilded.items[0].quality == 0
#     assert gilded.items[1].quality == 0
#     assert gilded.items[2].quality == 0

# def test_quality_limit():
#     '''Tests that the quality never increases above 50'''
#     items = [Item(name="Aged Brie", sell_in=2, quality=50), Item(name="Conjured Mana Cake", sell_in=3, quality=-3)]
#     gilded = GildedRose(items)
#     gilded.update_quality()
